...
1. **Check Vector Deployment**:
   ```bash
   kubectl get deployment vector-syslog -n grafana
   kubectl get pods -n grafana -l app=vector,component=syslog
   ```

2. **Check Syslog Service**:
   ```bash
   kubectl get svc vector-syslog -n grafana
...
3. **Check Vector Logs**:
   ```bash
   kubectl logs -n grafana -l app=vector,component=syslog
   ```
...
kubectl logs -n grafana -l app=vector,component=syslog --tail=50
...
1. **Log in to Grafana**:
   ...
   - Username: `admin`
   - Password: Auto-generated by Helm chart (`kubectl get secret grafana -n grafana -o jsonpath='{.data.admin-password}' | base64 -d`)
...
1. **Check Vector Pod Status**:
   ```bash
   kubectl get pods -n grafana -l app=vector,component=syslog
   kubectl describe pod -n grafana -l app=vector,component=syslog
   ```
...
3. **Verify Service**:
   ```bash
   kubectl get svc vector-syslog -n grafana
   kubectl describe svc vector-syslog -n grafana
   ```
...
1. **Check Vector Configuration**:
   ```bash
   kubectl get configmap vector-config -n grafana -o yaml
   ```

2. **Verify Loki Endpoint**:
   ```bash
   # Check if Loki distributor service is accessible (microservices mode)
   kubectl get svc loki-distributor -n grafana
   kubectl port-forward -n grafana svc/loki-distributor 3100:3100
...
1. **Check Vector Logs** for parsing errors:
   ```bash
   kubectl logs -n grafana -l app=vector,component=syslog | grep -i error
   ```
...
4. **Verify Socket Source Configuration**:
   ...
   - Check config: `kubectl get configmap vector-config -n grafana -o yaml`
